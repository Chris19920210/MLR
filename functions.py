...
def adam(VW, VU, m_w, m_u, v_w, v_u, beta1, beta2, it, alpha, epison):
    ...
    return m_w, m_u, v_w, v_u, alpha * (m_w_hat / (v_w_hat ** 0.5 + epison)), alpha * (
    m_u_hat / (v_u_hat ** 0.5 + epison))
...
def lbfgs(VW, VU, sList_w, sList_u, yList_w, yList_u, k, m, start):
    ...
    if ((sList_w[start] * yList_w[start] + sList_u[start] * yList_u[start]).sum() > 0):
        ...
        ro = (
        yList_w[(start - 1) % m] * sList_w[(start - 1) % m] + yList_u[(start - 1) % m] * sList_u[(start - 1) % m]).sum()
        print ("ro %f" % ro)

        for i in loop(L, start, 'left'):
            alpha = (sList_u[i] * q_u + sList_w[i] * q_w).sum() / (
            sList_u[i] * yList_u[i] + sList_w[i] * yList_u[i]).sum()
            print ("alpha %f" % alpha)
        ...
        for i in loop(L, start, 'right'):
            beta = (yList_u[i] * q_u + yList_w[i] * q_w).sum() / (
            sList_u[i] * yList_u[i] + sList_w[i] * yList_u[i]).sum()
